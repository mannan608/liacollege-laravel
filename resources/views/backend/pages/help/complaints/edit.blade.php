...
@section('content')
    ...
    <div class="mb-6">
        <a href="{{ role_route('role.help.complaints.show', ['complaint' => $complaint]) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 transition-colors">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Back to Complaint #{{ $complaint->id }}
        </a>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden">
        <div class="px-8 py-6 border-b border-gray-100 dark:border-white/5">
            <h3 class="text-xl font-semibold text-gray-800 dark:text-white/90">Edit Formal Complaint #{{ $complaint->id }}</h3>
        </div>

        <form method="POST" action="{{ role_route('role.help.complaints.update', ['complaint' => $complaint]) }}" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="pt-4 border-t border-gray-100 dark:border-white/5 flex justify-end gap-3">
                <a href="{{ role_route('role.help.complaints.show', ['complaint' => $complaint]) }}" class="px-5 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-lg transition-colors">
                    Cancel
                </a>
