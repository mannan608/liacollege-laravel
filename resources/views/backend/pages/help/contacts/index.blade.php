...
@section('content')
    <div class="">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
                <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Contact Admin Messages</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">View and manage messages sent by students to administrators.</p>
            </div>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        @forelse($messages as $message)
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-white/1 transition-colors">
                                <td class="px-5 py-4">
                                    <span class="px-2 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 rounded text-xs font-mono">#{{ $message->id }}</span>
                                </td>
                                <td class="px-5 py-4">
                                    <div>
                                        <div class="text-sm font-medium text-gray-700 dark:text-gray-200">{{ $message->user?->name ?? 'N/A' }}</div>
                                        <div class="text-xs text-gray-500 dark:text-gray-400">{{ $message->user?->email ?? '' }}</div>
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300 max-w-xs">
                                    <div class="truncate" title="{{ $message->message }}">
                                        {{ \Illuminate\Support\Str::limit($message->message, 80) }}
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-sm text-gray-600 dark:text-gray-300">
                                    {{ $message->course_title ?: ($message->recognised_code ?: 'N/A') }}
                                </td>
                                <td class="px-5 py-4">
                                    @php
                                        $statusColors = [
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-1.5">
                                        <a href="{{ role_route('role.help.contacts.show', ['message' => $message]) }}" class="p-2 text-gray-500 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-500/10 rounded-lg transition-all" title="View">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </a>
                                        <a href="{{ role_route('role.help.contacts.edit', ['message' => $message]) }}" class="p-2 text-gray-500 hover:text-green-600 hover:bg-green-50 dark:hover:bg-success-500/10 rounded-lg transition-all" title="Edit & Reply">
                                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <form action="{{ role_route('role.help.contacts.destroy', ['message' => $message]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this message?');">
                                            @csrf
                                            @method('DELETE')
