...
@section('content')
    <div x-data="lessonEditor()" class="max-w-5xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            ...
            <input type="text" x-model="lessonTitle"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <template x-for="type in resourceTypes" :key="type.id">
                    <button @click="toggleType(type.id)"
                        ...
                        <div :class="selectedTypes.includes(type.id) ? type.color : 'text-slate-400'"
                        ...
                        <span
                            :class="selectedTypes.includes(type.id) ? 'text-indigo-700 font-semibold' :
                                'text-slate-600 font-medium'"
                            class="text-sm" x-text="type.label"></span>

                        <!-- Checkmark badge when selected -->
                        <div x-show="selectedTypes.includes(type.id)" x-transition
        ...
        <!-- Video Section -->
        <div x-show="selectedTypes.includes('video')" x-cloak x-transition
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="addVideo()" type="button"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm cursor-pointer">
            ...
            <div class="space-y-4">
                <template x-for="(video, index) in videos" :key="'video-' + index">
                    <div
                        class="relative bg-slate-50 rounded-xl border border-slate-200 p-5 group hover:border-indigo-200 transition-colors">
                        <button @click="removeVideo(index)" type="button"
                            class="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors cursor-pointer">
                            ✕
                        </button>
                        <div class="flex items-center gap-3 mb-4">
                            <span
                                class="flex items-center justify-center w-8 h-8 bg-indigo-100 text-indigo-700 text-xs font-bold rounded-lg"
                                x-text="index + 1"></span>
                            <span class="text-sm font-semibold text-slate-500">Video <span x-text="index + 1"></span></span>
                        </div>
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Video
                                    Title</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">YouTube
                                    URL</label>
        ...
        <!-- Content Section -->
        <div x-show="selectedTypes.includes('content')" x-cloak x-transition
            class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
            ...
            <div class="space-y-4">
                <template x-for="(content, index) in contents" :key="'content-' + index">
                    <div
                        class="relative bg-slate-50 rounded-xl border border-slate-200 p-5 group hover:border-emerald-200 transition-colors">
                        <button @click="removeContent(index)" type="button"
                            class="absolute top-3 right-3 p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors cursor-pointer">
                            ✕
                        </button>
                        <div class="flex items-center gap-3 mb-4">
                            <span
                                class="flex items-center justify-center w-8 h-8 bg-emerald-100 text-emerald-700 text-xs font-bold rounded-lg"
                                x-text="index + 1"></span>
                            <span class="text-sm font-semibold text-slate-500">Content <span
                                    x-text="index + 1"></span></span>
                        </div>
                        <div class="space-y-4">
                            <div>
                                <label
                                    class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Content
                                    Title</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Description</label>
        ...
        <!-- File Section -->
        <div x-show="selectedTypes.includes('file')" x-cloak x-transition
            class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
            ...
            <div class="space-y-4">
                <template x-for="(file, index) in files" :key="'file-' + index">
                    <div
                        class="relative bg-slate-50 rounded-xl border border-slate-200 p-5 group hover:border-amber-200 transition-colors">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <span
                                class="flex items-center justify-center w-8 h-8 bg-amber-100 text-amber-700 text-xs font-bold rounded-lg"
                                x-text="index + 1"></span>
                            <span class="text-sm font-semibold text-slate-500">File <span
                                    x-text="index + 1"></span></span>
                        </div>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">File
                                    Title</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Upload
                                    File</label>
                                <div @dragover.prevent="file.dragOver = true" @dragleave.prevent="file.dragOver = false"
                                    ...
                                    <input type="file" :x-ref="'fileInput' + index"
                                        @change="handleFileSelect($event, index)" class="hidden"
                                        accept=".pdf,.doc,.docx,.ppt,.pptx">
                                    <p class="text-sm font-medium text-slate-700">
                                        <span x-show="!file.fileName">Drop your file here, or <span
                                                class="text-amber-600">browse</span></span>
                                        <span x-show="file.fileName" x-text="file.fileName"
                                            class="text-amber-700"></span>
                                    </p>
        ...
        <!-- Quiz Section -->
        <div x-show="selectedTypes.includes('quiz')" x-cloak x-transition
            class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-6">
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-bold text-slate-800 flex items-center gap-2">
                    <span class="text-violet-500">❓</span> Quiz Resources
                </h2>
                <button @click="addQuiz()" type="button"
                    class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm cursor-pointer">
            ...
            <div class="space-y-4">
                <template x-for="(quiz, index) in quizzes" :key="'quiz-' + index">
                    <div
                        class="relative bg-slate-50 rounded-xl border border-slate-200 p-5 group hover:border-violet-200 transition-colors">
                        ...
                        <div class="flex items-center gap-3 mb-4">
                            <span
                                class="flex items-center justify-center w-8 h-8 bg-violet-100 text-violet-700 text-xs font-bold rounded-lg"
                                x-text="index + 1"></span>
                            <span class="text-sm font-semibold text-slate-500">Quiz <span
                                    x-text="index + 1"></span></span>
                        </div>
                        <div class="space-y-4">
                            <div>
                                <label class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Quiz
                                    Title</label>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-medium text-slate-500 mb-1.5 uppercase tracking-wide">Question</label>
        ...
        <div class="flex items-center justify-end gap-3">
            <button @click="cancel()" type="button"
                class="px-6 py-2.5 text-slate-600 font-medium hover:bg-slate-100 rounded-lg transition-colors cursor-pointer">Cancel</button>
            <button @click="save()" type="button"
                class="px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-medium rounded-lg shadow-sm transition-colors cursor-pointer">Save
                Lesson</button>
        </div>
        ...
        function lessonEditor() {
            return {
                ...
                files: [{
                    ...
                    fileName: 'laravel-cheatsheet.pdf',
                    dragOver: false
                }],
                quizzes: [{
