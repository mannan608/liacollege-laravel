...
<div x-data="{
    ...
    eligibilityBaseUrl: {{ \Illuminate\Support\Js::from(url('/' . $role . '/eligibility')) }},
    ...
    confirmDelete() {
        ...
        <form x-ref="deleteForm" :action="rowToDelete ? (eligibilityBaseUrl + '/' + rowToDelete.id) : '#'" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
