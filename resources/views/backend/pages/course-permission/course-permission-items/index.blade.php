    ...
    <div id="content-access-tree" class="space-y-5 {{ $isFullAccess ? 'hidden' : '' }}">
            ...
            <div class="rounded-2xl border border-slate-200 overflow-hidden">
                <div class="flex items-center justify-between gap-4 border-b border-slate-200 bg-slate-50 px-4 py-3">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="categories[]" value="{{ $category->id }}" data-role="category"
                            data-category-id="{{ $category->id }}" @checked(in_array($category->id, $selectedCategories ?? [], true))
                        ...
                        <div class="px-4 py-4">
                            <div class="flex items-center justify-between gap-4">
                                <label class="flex items-center gap-3">
                                    <input type="checkbox" name="sections[]" value="{{ $section->id }}"
                                        data-role="section" data-category-id="{{ $category->id }}"
                                        data-section-id="{{ $section->id }}" @checked(in_array($section->id, $selectedSections ?? [], true))
                                        class="h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500">
                                    <span
                                        class="text-sm font-semibold text-slate-800">{{ $section->section_name }}</span>
                                </label>
                            </div>
                            ...
                            <div class="mt-3 space-y-2 pl-7">
                                @foreach ($section->rows as $row)
                                    <label
                                        class="flex items-start gap-3 rounded-xl border border-slate-100 px-3 py-2 hover:bg-slate-50">
                                        <input type="checkbox" name="rows[]" value="{{ $row->id }}"
                                            data-role="row" data-category-id="{{ $category->id }}"
                                            data-section-id="{{ $section->id }}" data-row-id="{{ $row->id }}"
        ...
        @if ($course->assignments->count())
            ...
            <div class="rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <div class="flex items-center justify-between bg-slate-50 px-4 py-3 border-b">

                    <label class="flex items-center gap-3">

                        {{-- CHANGED --}}
                        <input type="checkbox" data-role="assignment-group"
                ...
                <div class="space-y-2 p-4">
                    ...
                    @foreach ($course->assignments as $assignment)
                        <label class="flex items-center gap-3">
                            ...
                            {{-- CHANGED --}}
                            <input type="checkbox" name="assignments[]" value="{{ $assignment->id }}"
                                data-role="assignment" @checked(in_array($assignment->id, $selectedAssignments ?? []))
                                class="h-4 w-4 rounded border-slate-300 text-brand-600">
...
<script>
    (function() {
        ...
        const qSection = (catId, secId) =>
            tree.querySelector(
                `input[data-role="section"][data-category-id="${catId}"][data-section-id="${secId}"]`);
        ...
        const qRowsInSection = (catId, secId) =>
            tree.querySelectorAll(
                `input[data-role="row"][data-category-id="${catId}"][data-section-id="${secId}"]`);

        const qAssignmentGroup = () => tree.querySelector('input[data-role="assignment-group"]');

        const qAssignments = () => tree.querySelectorAll('input[data-role="assignment"]');
        ...
        tree.addEventListener('change', function(e) {
            ...
            if (role === 'category') {
            ...
            } else if (role === 'assignment-group') {

                qAssignments().forEach(item => {
                ...
                const group = qAssignmentGroup();

                const assignments = qAssignments();

                if (group) {

                    group.checked = Array.from(assignments)
                        .every(a => a.checked);

                    group.indeterminate =
                        Array.from(assignments).some(a => a.checked) &&
                        !group.checked;
...
if(group){
    ...
    if(assignments.length){
        ...
        group.checked =
            Array.from(assignments).every(a => a.checked);
