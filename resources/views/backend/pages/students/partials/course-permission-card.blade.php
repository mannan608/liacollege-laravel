                    ...
                    <div class="bg-brand-600  px-6 py-5">
                        <div class="flex items-center justify-between gap-4">
                            <div class="min-w-0">
                                ...
                                <p class="mt-1 text-xs font-medium text-white flex items-center gap-1.5">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                                    </svg>
                                    {{ $course->categories->count() }} Categories
                                </p>
                            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            @foreach ($course->categories as $category)
                                <div class="bg-white rounded-xl border border-slate-200 overflow-hidden"
                                    data-category="{{ $category->id }}" data-course="{{ $course->id }}">
                                    ...
                                    <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50">
                                        <label class="flex items-center gap-3 cursor-pointer min-w-0">
                                            <input type="checkbox" class="category-checkbox w-4 h-4 rounded border-2 border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500/20 focus:ring-offset-0 cursor-pointer"
                                                name="permissions[{{ $course->id }}][categories][]"
                                                value="{{ $category->id }}" data-course="{{ $course->id }}"
                                                data-category="{{ $category->id }}" @checked(in_array($course->id, $grantedCourses) || in_array($category->id, $grantedCategories))
                                    ...
                                    <div class="divide-y divide-slate-100">
                                        ...
                                        @foreach ($category->sections as $section)
                                            <div class="px-5 py-4" data-section="{{ $section->id }}" data-category="{{ $category->id }}" data-course="{{ $course->id }}">
                                                ...
                                                <div class="flex items-center justify-between mb-3">
                                                    <label class="flex items-center gap-3 cursor-pointer min-w-0">
                                                        <input type="checkbox" class="section-checkbox w-4 h-4 rounded border-2 border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500/20 focus:ring-offset-0 cursor-pointer"
                                                            ...
                                                            data-section="{{ $section->id }}" @checked(in_array($course->id, $grantedCourses) || in_array($category->id, $grantedCategories) || in_array($section->id, $grantedSections))
                                                ...
                                                <div class="space-y-2 pl-7">
                                                    ...
                                                    @foreach ($section->rows as $row)
                                                        <div class="flex items-start gap-3 py-2 hover:bg-slate-50/80 transition-colors duration-150 rounded-lg px-2 -mx-2">
                                                            <label class="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                                                                <input type="checkbox" class="row-checkbox w-4 h-4 rounded border-2 border-slate-300 text-brand-600 focus:ring-2 focus:ring-brand-500/20 focus:ring-offset-0 mt-0.5 cursor-pointer shrink-0"
                                                                    ...
                                                                    data-row="{{ $row->id }}" @checked(in_array($course->id, $grantedCourses) || in_array($category->id, $grantedCategories) || in_array($section->id, $grantedSections) || in_array($row->id, $grantedRows))
        ...
        return {
            init() {
                ...
                this.$nextTick(() => {
                    ...
                    document.querySelectorAll('.category-checkbox').forEach(category => {
                        this.updateCategory(category.dataset.category);
                    });
            ...
            toggleCourse(event) {
                ...
                document.querySelectorAll(
                    `[data-course="${courseId}"].category-checkbox,
                 .category-checkbox[data-course="${courseId}"]`
            ...
            toggleCategory(event) {
                ...
                document.querySelectorAll(
                    `.section-checkbox[data-category="${categoryId}"]`
                ).forEach(el => el.checked = checked);

                document.querySelectorAll(
                    `.row-checkbox[data-category="${categoryId}"]`
                ).forEach(el => el.checked = checked);
            ...
            updateCategory(categoryId) {
                ...
                const checkedSections = document.querySelectorAll(
                    `.section-checkbox[data-category="${categoryId}"]:checked`
                );

                const categoryCheckbox = document.querySelector(
                    `.category-checkbox[data-category="${categoryId}"]`
                );
            ...
            updateCourse(courseId) {
                ...
                const checkedCategories = document.querySelectorAll(
                    `.category-checkbox[data-course="${courseId}"]:checked`
                );
