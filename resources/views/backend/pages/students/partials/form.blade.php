@php
    ...
    $selectedSlotId = old('slot_id', $selectedSlotId ?? $student?->enrollments?->first()?->course_slot_id);
    ...
    <div class="grid gap-6 p-6 md:grid-cols-2">
        <div class="group">
            ...
            <input id="first_name" name="first_name" type="text"
                value="{{ old('first_name', $studentNameParts[0] ?? '') }}" required placeholder="Enter first name"
        ...
        <div class="group">
            ...
            <input id="last_name" name="last_name" type="text"
                value="{{ old('last_name', $studentNameParts[1] ?? '') }}" required placeholder="Enter last name"
                class="block w-full rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-600 dark:bg-gray-900 dark:text-white">
            @error('last_name')
                <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p>
            @enderror
    ...
    <script>
        ...
        function normalizeCheckoutSlots(slots) {
            return (slots || []).map((slot) => ({
                id: String(slot.id),
                name: `${slot.title ?? ('Slot #' + slot.id)} (${formatDate(slot.training_date)} ${slot.start_time} - ${slot.end_time})`,
            }));
        }
