...
@section('content')
    <div class="space-y-6">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Create a student account and enroll courses.</p>
        </div>

        <form method="POST" action="{{ role_route('role.students.store') }}" class="space-y-6">
            @csrf
            @include('backend.pages.students.partials.form', ['student' => null])
        </form>
        @if ($errors->any())
