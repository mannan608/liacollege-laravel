                ...
                @foreach ($menuGroup['items'] as $itemIndex => $item)
                    @if (isset($item['subItems']))
                        ...
                        @foreach ($item['subItems'] as $subItem)
                            @php($subItemUrl = MenuStudentPortal::itemUrl($subItem))
    ...
    <div class="pt-4 pb-3 flex"
        ...
        <a href="{{ role_route('role.dashboard') }}" class="flex items-center gap-3">
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="dark:hidden" src="{{ asset('logo.webp') }}" alt="Logo" width="60" height="auto" />
            <img x-show="$store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen"
                class="hidden dark:block" src="{{ asset('logo.webp') }}" alt="Logo" width="60"
                height="auto" /> 
            <img x-show="!$store.sidebar.isExpanded && !$store.sidebar.isHovered && !$store.sidebar.isMobileOpen"
                ...
                @foreach ($menuGroups as $groupIndex => $menuGroup)
                    <div>
                        ...
                        <ul class="flex flex-col gap-1">
                            @foreach ($menuGroup['items'] as $itemIndex => $item)
                                @php($itemUrl = MenuStudentPortal::itemUrl($item))
                                <li>
                                    @if (isset($item['subItems']))
                                        @php($subItemUrls = collect($item['subItems'])->map(fn ($subItem) => MenuStudentPortal::itemUrl($subItem))->filter(fn ($url) => $url !== '#')->values()->all())
